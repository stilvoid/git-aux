...
function copy(src, dest) {   
    ...
    Promise.wrap(fs.stat, src).then(function(err, stats) {
        var is, os;

        if(stats.isDirectory()) {
            Promise.wrap(fs.stat, dest).then(function(err, dest_stats) {
                function copy_files() {
                    return Promise.wrap(fs.readdir, src).then(function(err, files) {
                        var count = files.length;
                ...
                if(err) {
                    Promise.wrap(fs.mkdir, dest).then(copy_files);
